...
public:
   ...
   * @param[in] pVoc          词典
   * @param[in] bFixScale
   * 表示sim3中的尺度是否要计算,对于双目和RGBD情况尺度是固定的,s=1,bFixScale=true;而单目下尺度是不确定的,此时bFixScale=false,sim
   * 3中的s需要被计算
   */
  ...
  /** @brief
   * 由外部线程调用,请求复位当前线程.在回环检测复位完成之前,该函数将一直保持堵塞状态
   */
  void RequestReset();
...
protected:
   ...
   * \n \n
   * 注意以上匹配的结果均都存在成员变量mvpCurrentMatchedPoints中，实际的更新步骤见CorrectLoop()步骤3：Start
   ...
   * \n
   * 3. 通过MapPoints的匹配关系更新这些帧之间的连接关系，即更新covisibility
   * graph                                      \n
   * 4. 对Essential Graph（Pose
   * Graph）进行优化，MapPoints的位置则根据优化后的位姿做相对应的调整 \n
  ...
  /// 一个队列, 其中存储了参与到回环检测的关键帧
  /// (当然这些关键帧也有可能因为各种原因被设置成为bad,这样虽然这个关键帧还是存储在这里但是实际上已经不再实质性地参与到回环检测的过程中去了)
  ...
  /// 全局BA线程是否在进行
  bool mbRunningGBA;
  /// 全局BA线程在收到停止请求之后是否停止的比标志 //?
  /// 可是直接使用上面变量的逆不就可以表示了吗? //? 表示全局BA工作是否正常结束?
  bool mbFinishedGBA;
